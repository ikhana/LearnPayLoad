...
Create `src/endpoints/bulkPublish.ts`:
...
### 5b. Import into Posts

Open `src/collections/Posts.ts` and update the endpoints array:
...
export const Posts: CollectionConfig = {
  // ...
  endpoints: [searchPosts, bulkPublish],
}
```
...
```bash
git add src/endpoints/bulkPublish.ts src/collections/Posts.ts
